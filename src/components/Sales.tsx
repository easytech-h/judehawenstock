  ...
  const generateTicketPDF = () => {
    try {
      ...
      const tableData = currentSale.items.map((item: any) => {
        ...
        return [product?.name, item.quantity, `${item.price.toFixed(2)} HTG`, `${(item.quantity * item.price).toFixed(2)} HTG`];
      });
      ...
      doc.autoTable({
        ...
        body: tableData,
        foot: [
          ['', '', 'Subtotal:', `${currentSale.subtotal.toFixed(2)} HTG`],
          ['', '', 'Discount:', `${currentSale.discount.toFixed(2)} HTG`],
          ['', '', 'Total:', `${currentSale.total.toFixed(2)} HTG`],
          ['', '', 'Payment Received:', `${currentSale.paymentReceived.toFixed(2)} HTG`],
          ['', '', 'Change:', `${currentSale.change.toFixed(2)} HTG`],
        ],
        theme: 'grid',
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProducts.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{product.price.toFixed(2)} HTG</td>
        ...
        <div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {selectedProducts.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{product.price.toFixed(2)} HTG</td>
                    <td className="px-6 py-4 whitespace-nowrap">{(product.price * product.quantity).toFixed(2)} HTG</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xl font-bold">{total.toFixed(2)} HTG</span>
            </div>
            <div className="mb-4">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <span>{change.toFixed(2)} HTG</span>
            </div>
            <button
            ...
            <div className="mt-3">
              ...
              <div className="border p-4 mb-4">
                    ...
                    {currentSale.items.map((item: any, index: number) => {
                      ...
                      return (
                        <tr key={index} className="border-b">
                          ...
                          <td className="text-right">{item.price.toFixed(2)} HTG</td>
                          <td className="text-right">{(item.quantity * item.price).toFixed(2)} HTG</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <div className="flex justify-between mb-2">
                  <span>Subtotal:</span>
                  <span>{currentSale.subtotal.toFixed(2)} HTG</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span>Discount:</span>
                  <span>{currentSale.discount.toFixed(2)} HTG</span>
                </div>
                <div className="flex justify-between font-bold mb-2">
                  <span>Total:</span>
                  <span>{currentSale.total.toFixed(2)} HTG</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span>Payment Received:</span>
                  <span>{currentSale.paymentReceived.toFixed(2)} HTG</span>
                </div>
                <div className="flex justify-between font-bold">
                  <span>Change:</span>
                  <span>{currentSale.change.toFixed(2)} HTG</span>
                </div>
              </div>
